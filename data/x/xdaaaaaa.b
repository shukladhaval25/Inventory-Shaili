...
Module ModMain
    ...
    Public LicenseCount As Integer
    Public IsTempLicense As Boolean
    ...
    Public Logs As Logs = New Logs()
    Sub Main()
        Try
            Logger = New FileLogger
            conn = New OleDb.OleDbConnection
            DbConn()
            StrAppName = getCompanyName()
            myLogin = New frmLogin
            myLogin.ShowDialog()
        Catch ex As Exception
            Logs.ErrorMessage = ex.Message
            Logs.Details = ex.ToString
            Logs.Method = "Main"
    ...
    Public Sub DbConn()
        Try
            ...
            strConnString = "Provider=" & My.Settings.DBProvider & "; Data Source=" & My.Settings.DataSource & "; Persist Security Info =False"
        ...
        Catch ex As Exception
            ...
            Logger.WriteLog(logs)
            If show_error = True Then
            MsgBox("Unable to connect with database. Please verify your database setting", MsgBoxStyle.Critical)
            End If
        End Try
    End Sub
    ...
    Public Sub FillLookup(ByVal Strquery As String, ByVal strtable As String, ByVal strID As String, ByVal strName As String, ByRef lkupObj As DevExpress.XtraEditors.LookUpEdit, Optional ByVal strFld As String = "", Optional ByVal strsql As String = "")
        ...
        End Try
    End Sub
    Public Sub FillLookupWithMultipleValues(ByVal Strquery As String, ByVal strtable As String, ByVal strID As String, ByVal strName As String, ByRef lkupObj As DevExpress.XtraEditors.LookUpEdit, Optional ByVal strFld As String = "", Optional ByVal strsql As String = "")
        Try
            Dim ds As New DataSet
            'Dim adp As New OleDb.OleDbDataAdapter(Strquery, conn.ConnectionString)
            'adp.Fill(ds, strtable)
            ...
            For i As Integer = 0 To ds.Tables(0).Columns.Count - 1
                lkupObj.Properties.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo(ds.Tables(0).Columns(i).ToString))
            Next
            'lkupObj.Properties.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo(ds.Tables(0).Columns(0).ToString))
            'lkupObj.Properties.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo(ds.Tables(0).Columns(1).ToString))
            If strFld <> "" Then
    ...
    Private Function getCompanyName() As String
        ...
        Dim companyName As String = "Inventory Shaili System"
        Try
            Dim sql As String
            sql = "SELECT CompanyMaster.CompanyName FROM(CompanyMaster) where companyId ='1'"
            Dim cmd As OleDb.OleDbCommand
            cmd = New OleDb.OleDbCommand(sql, conn)
            companyName = IIf(IsDBNull(cmd.ExecuteScalar), companyName, cmd.ExecuteScalar)
        Catch
        End Try
        Return companyName
    End Function
