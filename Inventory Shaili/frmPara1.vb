...
#End Region
        ...
        Try
            Select Case strReport
                ...
                Case "DW_PackingVI"
                    ...
                    grpdate.Visible = True
                Case "SemiFG_RejectionComponent"
                    lblName.Text = "CMIS No"

                    Dim strsql As String = "SELECT SEMI_Material_Issue_Master.MIS_NO, Semi_MIS_Return_Master.ID" _
                        & " FROM SEMI_Material_Issue_Master INNER JOIN Semi_MIS_Return_Master " _
                        & " ON SEMI_Material_Issue_Master.MIS_NO = Semi_MIS_Return_Master.MIS_NO" _
                        & " WHERE (((SEMI_Material_Issue_Master.CY)='" & strCY & "'))"

                    FillLookup(strsql, "CMIS", "CMIS No", "Rejection ID", lkupName)
                    grpdate.Visible = False
                    lblGroup.Visible = False
                    lkupGroup.Visible = False
                Case "FG_RejectionComponent"
                    lblName.Text = "MIS No"

                    Dim strsql As String = "SELECT Material_Issue_Master.MIS_NO,MIS_Return_Master.ID" _
                                        & " FROM Material_Issue_Master INNER JOIN MIS_Return_Master " _
                                        & " ON Material_Issue_Master.MIS_NO = MIS_Return_Master.MIS_NO" _
                                        & " WHERE (((Material_Issue_Master.CY)='" & strCY & "'))"

                    FillLookup(strsql, "MIS", "MIS No", "Rejection ID", lkupName)
                    grpdate.Visible = False
                    lblGroup.Visible = False
                    lkupGroup.Visible = False
                Case "LineClearance", "QC_WO_Register"
                    lblName.Text = "W.O No"

                    Dim strsql As String = "SELECT REQMASTER.WORKORDERNO, REQMASTER.WORKORDERNO " _
                            & " FROM FGMaster INNER JOIN REQMASTER ON FGMaster.FID = REQMASTER.FID " _
                            & " WHERE (((REQMASTER.CY)='" & strCY & "') AND WORKORDERNO <> '');"

                    FillLookup(strsql, "WO", "Work Order No", "Finish Good", lkupName)
    ...
    Private Sub cmdOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOK.Click
        Try
            If (strID = Nothing) Then
                strID = ""
            End If
            If (strGroupID = Nothing) Then
                strGroupID = ""
            End If
            If (strGVUserID = Nothing) Then
                strGVUserID = ""
            End If
            Select Case strReport
                Case "CW_BW_Sales"
                ...
                Case "BWFG", "DW_PackingVI"
                    DialogResult = DialogResult.OK   ' Windows.Forms.DialogResult.OK
                Case "SemiFG_RejectionComponent"
                    Dim myPara As New frmPara
                    myPara.showSemiMISRejectionReport(strID)
                Case "FG_RejectionComponent"
                    Dim myPara As New frmPara
                    myPara.showMISRejectionReport(strID)
                Case "LineClearance", "QC_WO_Register"
    ...
    Private Sub fillGroupListBasedOnName()
        If (stractivetable = Nothing) Then
            Return
        End If
        If stractivetable.Equals("SemiFG_Wise_RM_Rejection") Then
            Dim strQuery As String = "SELECT RMItemMaster.RMItemName, RMItemMaster.RID" _
                & " FROM ((SemiFGMaster INNER JOIN SemiFGBatchMaster ON SemiFGMaster.SFID = SemiFGBatchMaster.SFID) " _
                & " INNER JOIN SemiFGBatchdetail ON SemiFGBatchMaster.BatchID = SemiFGBatchdetail.BatchID) " _
                & " INNER JOIN RMItemMaster ON SemiFGBatchdetail.RID = RMItemMaster.Rid " _
                & " WHERE (((SemiFGMaster.SFID)='" & strID & "'));"
            FillLookup(strQuery, "SemiFG", "Semi Finish Good", "SFID", lkupGroup)
        ElseIf stractivetable.Equals("FG_Wise_RM_Rejection") Then
            Dim strQuery As String = "SELECT RMItemMaster.RMItemName, RMItemMaster.RID" _
                & " FROM ((FGMaster INNER JOIN BatchMaster ON FGMaster.FID = BatchMaster.FID) " _
                & " INNER JOIN Batchdetail ON BatchMaster.BatchID = Batchdetail.BatchID) " _
                & " INNER JOIN RMItemMaster ON Batchdetail.RID = RMItemMaster.Rid " _
                & " WHERE (((FGMaster.FID)='" & strID & "'));"
            FillLookup(strQuery, "FG", "Finish Good", "FID", lkupGroup)
        End If
    End Sub
