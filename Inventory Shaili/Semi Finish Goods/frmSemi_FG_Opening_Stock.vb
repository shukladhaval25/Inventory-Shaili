Public Class frmSemi_FG_Opening_Stock
...
#End Region
    ...
    Private Sub frmFG_OpeningStock_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            FillLookup("SELECT SemiFGMaster.FGName, SemiFGMaster.SFID, UnitMaster.UnitName, UnitMaster.UnitID " _
            & " FROM SemiFGMaster INNER JOIN UnitMaster ON SemiFGMaster.UnitID = UnitMaster.UnitID", "SemiFGMaster", "Product Name", "SFID", lkupName)
            ...
            If strcheck = "Edit" Then
                ...
                cmd.CommandText = "select * from Semi_FG_Opening_Stock where Id = " & ModMain.strgridid
                cmd.Connection = conn
    ...
    Private Sub cmdsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdsave.Click
        Try
            ...
            If strcheck = "Add" Then
                ...
                cmd.CommandText = "Select max(ID) from Semi_FG_Opening_Stock"
                intID = IIf(IsDBNull(cmd.ExecuteScalar), 0, cmd.ExecuteScalar)
                intID = intID + 1
                'strFID = "FG00000090"
                'strUID = "UT0001"
                cmd.CommandText = "insert into Semi_FG_Opening_Stock values (" & intID & ",'" & strFID & "','" & strUID & "','" & Replace(txtFGVINO.Text, "'", "''") & "'," & txtQty.Text & ",'" & dtpdate.Text & "','" & strCY.Trim & "')"
                cmd.ExecuteNonQuery()

                cmd.CommandText = "Insert into SemiFGStock values ('" & strFID & "','" & dtpdate.Text & "','" & Replace(txtFGVINO.Text, "'", "''") & "','Opening'," & txtQty.Text & ",0,0," & txtQty.Text & ",'" & strCY.Trim & "')"
                cmd.ExecuteNonQuery()
            ...
            ElseIf strcheck = "Edit" Then
                ...
                cmd.CommandText = "Update Semi_FG_Opening_Stock  set Batch_No = '" & Replace(txtFGVINO.Text.Trim, "'", "''") & "', Opening_Qty = " & txtQty.Text & ", Register_Date ='" & dtpdate.Text & "' where ID =" & txtID.Text & " and CY ='" & strCY.Trim & "'"
                cmd.ExecuteNonQuery()

                cmd.CommandText = "Update SemiFGStock set Batch_NO='" & txtFGVINO.Text & "' where SFID ='" & strFID & "' and Batch_No ='" & strFGVINO & "' and CY ='" & strCY.Trim & "'"
                cmd.ExecuteNonQuery()
    ...
    Public Function CheckData() As Boolean
        Try
            ...
            cmd = New OleDb.OleDbCommand
            cmd.Connection = conn
            cmd.CommandText = "SELECT Count(Semi_FG_Opening_Stock.SFGVI_NO) AS CountOfSFGVI_NO, Semi_FG_Opening_Stock.SFID, Semi_FG_Opening_Stock.SFGVI_NO" _
                & " FROM SemiFGMaster INNER JOIN Semi_FG_Opening_Stock ON SemiFGMaster.SFID = Semi_FG_Opening_Stock.SFID " _
                & " GROUP BY Semi_FG_Opening_Stock.SFID, Semi_FG_Opening_Stock.SFGVI_NO " _
                & " HAVING (((Semi_FG_Opening_Stock.SFID)='" & strFID & "') AND ((Semi_FG_Opening_Stock.SFGVI_NO)= '" & txtFGVINO.Text.Trim & "'));"
    ...
    Private Sub bind_data()
        Try
            ...
            If IsDBNull(dr("SFid").ToString) = True Then
                lkupName.EditValue = ""
            Else
                lkupName.EditValue = dr("SFid").ToString
            End If

            'FGVI_NO
            If IsDBNull(dr("SFGVI_NO").ToString) = True Then
                txtFGVINO.Text = ""
            Else
                txtFGVINO.Text = dr("SFGVI_NO").ToString
