...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.grpbatch.Size = New System.Drawing.Size(735, 390)
        ...
        'btnReport
        '
        Me.btnReport.Location = New System.Drawing.Point(8, 360)
        ...
        'cmdsave
        '
        Me.cmdsave.Location = New System.Drawing.Point(568, 358)
        ...
        Me.cmdclose.Location = New System.Drawing.Point(648, 358)
        ...
        'PanelControl1
        '
        Me.PanelControl1.Controls.Add(Me.rgrpProduct)
        ...
        Me.PanelControl1.Size = New System.Drawing.Size(715, 224)
        ...
        '
        Me.cboUOM.EditValue = ""
        Me.cboUOM.Location = New System.Drawing.Point(406, 18)
        ...
        Me.lkupItemName.Location = New System.Drawing.Point(166, 18)
        ...
        '
        Me.txtqty.EditValue = ""
        Me.txtqty.Location = New System.Drawing.Point(534, 18)
        ...
        Me.lblqty.Location = New System.Drawing.Point(502, 18)
        ...
        Me.gdbatchdetail.Location = New System.Drawing.Point(8, 61)
        Me.gdbatchdetail.MainView = Me.gvbatch
        Me.gdbatchdetail.Name = "gdbatchdetail"
        Me.gdbatchdetail.Size = New System.Drawing.Size(690, 155)
        ...
        'cmdAdd
        '
        Me.cmdAdd.Location = New System.Drawing.Point(594, 18)
        ...
        'cmdDelete
        '
        Me.cmdDelete.Location = New System.Drawing.Point(650, 18)
        ...
        Me.lblRaw.Location = New System.Drawing.Point(126, 18)
        ...
        Me.lblfgname.Text = "Finish Good Name:"
        '
        'lblbatchid
        '
        ...
        Me.rgrpProduct.Location = New System.Drawing.Point(6, 5)
...
#End Region
    ...
    Private Sub frmbatch_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If strcheck = "Add" Then
                txtbatchid.Text = GetMaxNo("batchid", "batchmaster", "BP", 10)
            ...
            If strcheck = "Edit" Then
                ...
                cmd.CommandText = "SELECT Batchmaster.BatchID, FGMaster.FGName, Batchmaster.Qty,BATCHMASTER.FID " _
                    & " FROM Batchmaster INNER JOIN FGMaster ON Batchmaster.FID = FGMaster.FID " _
                    & " WHERE (((Batchmaster.BatchID)='" & ModMain.strgridid & "'))"
                ...
                Dim adp As New OleDb.OleDbDataAdapter("SELECT  Batchdetail.RID, RMItemMaster.RMItemName as [Item Name], Batchdetail.UnitID, UnitMaster.UnitName as Unit, " _
                                & " Batchdetail.Qty As Qty, 'RM' as ProductType " _
                                & " FROM ((Batchmaster INNER JOIN Batchdetail ON Batchmaster.BatchID = Batchdetail.BatchID) INNER JOIN RMItemMaster ON Batchdetail.RID = RMItemMaster.RId) INNER JOIN UnitMaster ON Batchdetail.UnitID = UnitMaster.UnitID " _
                                & " WHERE Batchmaster.BatchID='" & ModMain.strgridid & "'", conn)
                adp.Fill(dt)

                Dim adpSemi As New OleDb.OleDbDataAdapter("SELECT  SEMI_Batchdetail.SFID as RID , SEMIFGMaster.FGName as [Item Name], SEMI_Batchdetail.UnitID, UnitMaster.UnitName as Unit, " _
                                & " SEMI_Batchdetail.Qty as Qty, 'SFG' as ProductType " _
                                & " FROM ((Batchmaster INNER JOIN SEMI_Batchdetail ON Batchmaster.BatchID = SEMI_Batchdetail.BatchID) INNER JOIN SEMIFGMaster ON SEMI_Batchdetail.SFID = SEMIFGMaster.SFId) INNER JOIN UnitMaster ON SEMI_Batchdetail.UnitID = UnitMaster.UnitID " _
                                & " WHERE Batchmaster.BatchID='" & ModMain.strgridid & "'", conn)
    ...
    Private Sub cbofgname_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbofgname.GotFocus
        Try
            FillCombox("fgmaster", "fgname", cbofgname)
        Catch ex As Exception
            If show_error Then
                MsgBox(ex.ToString)
            End If
    ...
    Private Sub cbofgname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cbofgname.Validating
        ...
        If IsDuplicate(strSql) = False Then
            cmd = New OleDb.OleDbCommand
            cmd.Connection = conn
            cmd.CommandText = "select fid from fgmaster where fgname = '" & cbofgname.Text & "'"
        ...
        Try
            If strcheck = "Add" Then
                If ISVALIDATE() = True Then
                    ...
                    cmd.CommandText = "insert into batchmaster(batchid,fid,qty)" & _
                    ...
                    For i = 0 To gvbatch.RowCount - 1
                        If (gvbatch.GetRowCellValue(i, "ProductType") = "RM") Then
                            cmd.CommandText = "insert into batchdetail(batchid,FID,Rid,unitid,qty)" & _
                        ...
                        Else
                            cmd.CommandText = "insert into SEMI_Batchdetail(batchid,FID,SFID,unitid,qty)" & _
                           "values('" & batchmasterid & "','" & strfgid.Trim & "','" & gvbatch.GetRowCellValue(i, "RID") & "','" & gvbatch.GetRowCellValue(i, "UnitID") & "'," & gvbatch.GetRowCellValue(i, "Qty") & ")"
                        End If
                        cmd.Connection = conn
                        cmd.ExecuteNonQuery()
            ...
            ElseIf strcheck = "Edit" Then
                ...
                cmd.CommandText = "Delete from Batchdetail where BatchID = '" & txtbatchid.Text & "'"
                cmd.ExecuteNonQuery()
                cmd.CommandText = "Delete from SEMI_Batchdetail where BatchID ='" & txtbatchid.Text & "'"
                cmd.ExecuteNonQuery()
    ...
    Private Sub cboUOM_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboUOM.GotFocus
        Try
            ...
            If strItemID <> "" Then
                ...
                Else
                    cmd.CommandText = "SELECT UnitMaster.UnitName, SemiFGMaster.SFGID " _
                       & " FROM UnitMaster INNER JOIN SemiFGMaster ON  UnitMaster.UnitID = SemiFGMaster.SFID" _
                       & " WHERE (((SemiFGMaster.SFID)='" & strItemID & "'))"
                End If
                cmd.Connection = conn
                ...
                Loop
                drTemp.Close()
                If (rgrpProduct.SelectedIndex = 1) Then
                    cmd.CommandText = "SELECT UnitMaster.UnitName, SemiFGMaster.SFGID" _
    ...
    Private Sub FillProductCombo()
        If (rgrpProduct.SelectedIndex = 0) Then
            FillLookup("Select RMItemName,Rid from RMItemMaster", "RMItemMaster", "Item Name", "RID", lkupItemName)
        Else
            FillLookup("Select FGName,SFID from SemiFGMaster", "SemiFGMaster", "Item Name", "SFID", lkupItemName)
        End If
    End Sub
