...
Public Class frmRejectionComponent
    ...
    Private Sub frmRejectionComponent_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ElseIf strcheck = "Edit" Then
            Dim strsql = "SELECT MIS_Return_Master.ID, MIS_Return_Master.MIS_NO, MIS_Return_Master.Return_Date " _
                & " FROM(MIS_Return_Master) WHERE (((MIS_Return_Master.ID)=" & ModMain.strgridid & "));"
    ...
    Private Sub getMISInformation(ByVal misno As String)
        Dim strsql As String = "SELECT Material_Issue_Master.MIS_NO, Material_Issue_Master.MIS_DATE, FGMaster.FGName," _
                            & " FGMaster.FID, Material_Issue_Master.FG_QTY, REQMASTER.WORKORDERNO" _
                            & " FROM (FGMaster INNER JOIN REQMASTER ON FGMaster.FID = REQMASTER.FID) " _
                            & " INNER JOIN Material_Issue_Master ON REQMASTER.RQID = Material_Issue_Master.RQID" _
    ...
    Private Function getRawMaterial() As DataTable
        Dim sqlstr As String = "SELECT Material_Issue_Master.MIS_NO, Material_Issue_Detail.RID, RMItemMaster.RMItemName, Material_Issue_Detail.RMVI_NO, Material_Issue_Detail.Issued_Qty,'RM' as ProductType" _
            & " FROM (Material_Issue_Master INNER JOIN Material_Issue_Detail ON Material_Issue_Master.MIS_NO = Material_Issue_Detail.MIS_NO) INNER JOIN RMItemMaster ON Material_Issue_Detail.RID = RMItemMaster.Rid " _
            & " WHERE (((Material_Issue_Master.MIS_NO)='" + cmbMIS.Text + "'))"
    ...
    Private Function getSemiFGMaterial() As DataTable
        Dim sqlstr As String = "SELECT Material_Issue_Master.MIS_NO, Material_Issue_Detail.RID, SemiFGMaster.FGName as RMItemName, Material_Issue_Detail.RMVI_NO, Material_Issue_Detail.Issued_Qty,'SFG' as ProductType" _
            & " FROM (Material_Issue_Master INNER JOIN Material_Issue_Detail ON Material_Issue_Master.MIS_NO = Material_Issue_Detail.MIS_NO) INNER JOIN SemiFGMaster ON Material_Issue_Detail.RID = SemiFGMaster.SFID " _
            & " WHERE (((Material_Issue_Master.MIS_NO)='" + cmbMIS.Text + "'))"
        Dim adp = New OleDb.OleDbDataAdapter(sqlstr, conn)
        Dim dttemp As New DataTable
        adp.fill(dttemp)
    ...
    Private Sub bind_Masterdata(ByVal dr As OleDb.OleDbDataReader)
        ...
        lblFGVal.Tag = dr("FID").ToString()
        lblQtyVal.Text = dr("FG_QTY").ToString()
        txtWorkOrderVal.Text = dr("WORKORDERNO").ToString()
    End Sub
    ...
    Private Sub bind_DataFromMaterialReturn()
        For i As Int16 = 0 To gvMatIssue.RowCount - 1
            ...
            cmd.Connection = conn
            cmd.CommandText = "Select * from MIS_Return_Detail where MIS_NO ='" & gvMatIssue.GetRowCellValue(i, "MIS_NO") & _
            ...
            '    cmd.CommandText = "insert into MIS_Return_Detail values(" & maxId & ",'" & cmbMIS.Text & "','" & gvMatIssue.GetRowCellValue(i, "RID") & "','" & gvMatIssue.GetRowCellValue(i, "Operation Type") & _
            ...
            Else
                ...
                Else
                    strSql = "SELECT Semi_FG_Opening_Stock.SFGVI_NO as Expr1000 , SemiFGStock.Balance_Qty, Semi_FG_Opening_Stock.SFGVI_NO as [RMVI_NO]" _
                       ...
                       & " WHERE (((Semi_FG_Opening_Stock.SFID)='" & gvMatIssue.GetRowCellValue(e.RowHandle, gvMatIssue.Columns(1)) & "') AND ((SemiFGStock.CY)='" & strCY.Trim & "')) " _
    ...
    Private Sub cmdsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdsave.Click
        Try
            If strcheck = "Add" Then
                cmd = New OleDb.OleDbCommand
                cmd.Connection = conn
                cmd.CommandText = "insert into MIS_Return_Master ( MIS_NO, Return_Date, CY ) values ('" + cmbMIS.Text + "','" + dtpRejectionDate.Text + "','" + strCY + "')"
                cmd.Transaction = conn.BeginTransaction
                cmd.ExecuteNonQuery()

                cmd.CommandText = "Select Max(ID) from MIS_Return_Master"
                ...
                For i As Int16 = 0 To gvMatIssue.RowCount - 1
                    If (IsDBNull(gvMatIssue.GetRowCellValue(i, "Operation Type")) = False AndAlso IsDBNull(gvMatIssue.GetRowCellValue(i, "Operation_RMVI_NO")) = False AndAlso IsDBNull(gvMatIssue.GetRowCellValue(i, "Operation_Qty")) = False) Then
                        ...
                        If (gvMatIssue.GetRowCellValue(i, "RID").ToString.StartsWith("RM")) Then
                            ...
                            '    If (gvMatIssue.GetRowCellValue(i, "Operation Type") = "Return") Then
                            '        cmd.CommandText = "Update SemiFGStock set Ret_Qty = " & gvMatIssue.GetRowCellValue(i, "Operation_Qty") & ",Balance_Qty = Balance_Qty + " & gvMatIssue.GetRowCellValue(i, "Operation_Qty") & _
    ...
    Private Sub cmdReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdReport.Click
        Try
            ...
            myPara.showMISRejectionReport(ModMain.strgridid.Trim)
            myPara.Close()
        Catch ex As Exception
            If show_error Then
                MsgBox(ex.ToString)
